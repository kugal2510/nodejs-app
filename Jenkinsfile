    ...
    stages {
        ...
        stage('Pull') {
            steps {
                ...
                sh 'ssh -v root@5.42.74.179 "docker stop $(docker ps -a -q) && docker container rm $(docker images -q) && sleep 10 && docker login 79.137.248.252:8083 -u jenkins --password-stdin < ./pass && docker pull 79.137.248.252:8083/myapp_nodejs && docker run -it --detach -p 3000:3000 79.137.248.252:8083/myapp_nodejs:latest"'
                }
            }
        }
    }
